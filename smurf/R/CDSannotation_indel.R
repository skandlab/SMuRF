...
CDSannotation_indel = function(x, tbi, predicted, build, change.build){
  ...
  annotate.vcf=function(filename,build,mut.subset){
    ...
    if(length(gr)==length(vcf)){
      ...
      print("CDS indel annotation error") # need to fix this if there's error
    ...
    #tryCatch({
      ...
      #mutations.orig=myresults$smurf_indel$predicted_indel
      ...
      #use only START_POS_REF as ref to eliminate range generated by deletions in END_POS_REF
      mutations=with(mutations.orig,GRanges(Chr,IRanges(START_POS_REF,START_POS_REF),REF=REF,ALT=ALT,REF_MFVdVs=REF_MFVdVs,ALT_MFVdVs=ALT_MFVdVs,
      ...
      if (length(mutations)!=0){
        ...
        mut=mut[,-which(colnames(mut) %in% c("end","width","strand"))]
        mutations.orig=mutations.orig[,c("Chr","START_POS_REF","END_POS_REF","REF","ALT","REF_MFVdVs","ALT_MFVdVs",
                                         ...
                                         "REGION","SMuRF_score")]
        colnames(mut)[c(1:17)]=colnames(mutations.orig)[c(1:2,4:18)]
        mut$Chr=as.character(mut$Chr)
        mutations.orig$Chr=as.character(mutations.orig$Chr)
        mutations.orig=merge(mut,mutations.orig,by=c("Chr","START_POS_REF","REF","ALT","REF_MFVdVs","ALT_MFVdVs",
                                                     "FILTER_Mutect2","FILTER_Freebayes","FILTER_Vardict","FILTER_Varscan","FILTER_Strelka2",
                                                     "Sample_Name","Alt_Allele_Freq",
