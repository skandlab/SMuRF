...
snvRFparseall = function(a){
  ...
  #mutect_cds<-mutect_cds[mutect_cds$sampleNames == samples(header(a[[2]]))[1],]
  ...
  #vardict_cds<-vardict_cds[vardict_cds$sampleNames == samples(header(a[[2]]))[1],] #choose only the tumor sample values (1 or 2) eg.icgc_cll-T
  ...
  #### Merge 4 callers with proper coordinates ####
  ...
  #df <- final[,c(1:2,6:dim(final)[2])]
  #write.csv(df, 'snv_parse.csv',row.names = F)
  #df <- h2o.importFile(path = normalizePath("snv_parse.csv"),header=T)
  
  final<-as.matrix(final)
  y <- list(final)
  names(y)<- c("allfeatures_snv")
  return(y)
  
  
  #write.table(final , file = "snv-parse-allfeatures.txt", sep = "\t", quote = FALSE, row.names = FALSE)
  #x = read.table('snv-parse-allfeatures.txt',header=T,sep = "\t")
  #write.csv(x, 'snv-parse-allfeatures.csv',row.names = F)
