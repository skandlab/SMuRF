...
snvRFparse = function(a){
  ...
  mutect<-mutect[,c("seqnames","start","end", "ref", "alt", "MQ","MQRankSum","TLOD","NLOD","refDepth", "altDepth", "AF")]
  mutect<-mutect[grep("GL*", mutect$seqnames, invert=TRUE),]
  colnames(mutect)<-c("X.CHROM", "POS", "END_POS_Mutect2", "REF_Mutect2", "ALT_Mutect2", "m2_MQ","m2_MQRankSum","m2_TLOD","m2_NLOD","m2_refDepth", "m2_altDepth","m2_AF")
  
  freebayes<-as.data.frame(a[[3]][isSNV(a[[3]], singleAltOnly=FALSE)],row.names=NULL)
  freebayes<-freebayes[,c("seqnames","start","end", "ref", "alt","MQM","MQMR","RO","AO")]
  freebayes<-freebayes[grep("GL*", freebayes$seqnames, invert=TRUE),]
  colnames(freebayes)<-c("X.CHROM", "POS", "END_POS_Freebayes", "REF_Freebayes", "ALT_Freebayes","f_MQM","f_MQMR", "f_refDepth","f_altDepth")
  ...
  vardict<-vardict[,c("seqnames","start","end", "ref", "alt", "SSF","refDepth", "altDepth", "AF")]
  vardict<-vardict[grep("GL*", vardict$seqnames, invert=TRUE),]
  colnames(vardict)<-c("X.CHROM", "POS", "END_POS_Vardict", "REF_Vardict", "ALT_Vardict", "vd_SSF","vd_refDepth", "vd_altDepth", "vd_AF")
  ...
  snvraw <- list(raw, coordinates[,c("X.CHROM","POS")])
  names(snvraw)<- c("raw_snv","coordinates_snv")
  return(snvraw)
